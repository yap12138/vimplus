...
_debug = int(os.environ.get('YCM_EXTRA_DEBUG', '0'))

def Log(msg):
    if _debug != 1:
        return 

    f = open(os.path.join(g_ycm_dir, "debug.log"), 'a+')
    f.write(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f') + '|' + msg + '\n')
    f.close()

g_home = os.environ.get("HOME")
g_ycm_dir = os.path.join(g_home, '.ycm')
...
try:
    db = dbm.open(os.path.join(g_ycm_dir, 'cache'), "c")
except:
...
if not gcc_search_dirs:
    cpp = os.path.join(g_ycm_dir, 'tmp.cpp')
    f = open(cpp, 'w')
    f.write('#include <iostream>\n')
    f.write('main() {}')
    f.close()
    r, w, e = os.popen3('g++ -std=c++11 -v -H ' + cpp + ' -o /dev/null')
...
ycm_flags_config = LoadFlagsConfig(os.path.join(g_ycm_dir, 'flags.json'))

g_default_flag_config = os.environ.get('YCM_DEFAULT_FLAG_CONFIG', '/usr/local/share/vim/conf/tools/ycm/flags.json')
Log('YCM_DEFAULT_FLAG_CONFIG = %s' % g_default_flag_config)

if g_default_flag_config != '':
    Log("load default flags config from %s" % g_default_flag_config)
    ycm_flags_config.extend(LoadFlagsConfig(g_default_flag_config))

g_make_support = os.environ.get('YCM_MAKE_SUPPORT', 'both') # make|cmake|both
...
def GetFlagFromConfig(filename):
    dirname = os.path.dirname(filename)
    for v in ycm_flags_config:
        if not v:
            continue
        t = v.get('type')
        p = v.get('path')
        f = v.get('flag')
        Log('try config: type:%s path:%s' % (t, p))
        if t == 'dir' and dirname == p:
            return f
        elif t == 'file' and filename == p:
            return f
        elif t == 'prefix' and filename.startswith(p):
            return f
        elif t == 're' and re.match(p, filename):
            return f
    Log('config not found for %s' % filename)
    return False
...
def FlagsForFile( filename, **kwargs ):
    ...
    conf_flags = GetFlagFromConfig(filename)
    #conf = False
    if conf_flags:
        final_flags.extend(conf_flags)

    else:
        if g_make_support == 'make' or g_make_support == 'both':
            # parse makefile
        ...
        if g_make_support == 'cmake' or g_make_support == 'both':
            # parse cmake
...
if __name__ == '__main__':
    # test mk_flags
    print FlagsForFile(os.path.join(g_ycm_dir, 'tmp.cpp'))
